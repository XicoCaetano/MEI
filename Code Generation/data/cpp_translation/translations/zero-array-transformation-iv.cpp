...
public:
    int minZeroArray(vector<int> nums, vector<vector<int>> queries) {
        bool all_zero = true;
        for (int x : nums) if (x != 0) { all_zero = false; break; }
        if (all_zero) return 0;
        int n = (int)nums.size();
        vector<unordered_set<int>> subsetSums(n);
        for (int i = 0; i < n; ++i) subsetSums[i].insert(0);
        for (size_t k = 0; k < queries.size(); ++k) {
            int l = queries[k][0], r = queries[k][1], val = queries[k][2];
            for (int i = l; i <= r; ++i) {
                vector<int> newSums;
                newSums.reserve(subsetSums[i].size());
                for (int s : subsetSums[i]) newSums.push_back(s + val);
                for (int s : newSums) subsetSums[i].insert(s);
            }
            bool ok = true;
            for (int i = 0; i < n; ++i) {
                if (!subsetSums[i].count(nums[i])) { ok = false; break; }
            }
            if (ok) return (int)k + 1;
        }
        return -1;
    }
};
