...
public:
    ...
    int _lastModifiablePosition(int i, int m, const vector<bool>& modifiable) {
        ...
        for (int j = 0; j < m; ++j) {
            int pos = i + j;
            if (modifiable[pos]) pos_found = pos;
        }
        return pos_found;
    }
    ...
    string generateString(string str1, string str2) {
        ...
        int m = (int)str2.size();
        int sz = n + m - 1;
        // 0 means unset; otherwise the assigned character.
        string ans(sz, '\0');
        ...
        for (int i = 0; i < n; ++i) {
            if (str1[i] == 'F' && _match(ans, i, str2)) {
                int mp = _lastModifiablePosition(i, m, modifiable);
                if (mp == -1) return "";
                ans[mp] = 'b';
                modifiable[mp] = false;
            }
