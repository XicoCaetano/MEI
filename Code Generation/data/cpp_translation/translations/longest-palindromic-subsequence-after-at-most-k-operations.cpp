...
public:
    int longestPalindromicSubsequence(string s, int k) {
        ...
        function<int(int,int,int)> dfs = [&](int i, int j, int kk) -> int {
            if (i > j) return 0;
            if (i == j) return 1;
            if (dp[i][j][kk] != -1) return dp[i][j][kk];
            int res = max(dfs(i+1, j, kk), dfs(i, j-1, kk));
            int d = abs((int)s[i] - (int)s[j]);
            int t = min(d, 26 - d);
            if (t <= kk) {
                int new_kk = kk - t;
                // Clamp to [0, k]: more ops than k never helps; t<0 (non-alpha
                // chars) would otherwise overflow the dp array.
                if (new_kk > k) new_kk = k;
                if (new_kk < 0) new_kk = 0;
                res = max(res, dfs(i+1, j-1, new_kk) + 2);
            }
            return dp[i][j][kk] = res;
        };
        return dfs(0, n-1, k);
    }
};
